...
class Regions {
  ...
  Future<List<Kelurahan>> getActiveKelurahan() async {
    try {
      Response response =
          await _dio.get("${dotenv.env['API_HOST']}/sub_district/active");
      ...
      var listKelurahan = (data['data'] as List<dynamic>)
          .map(
            (e) => Kelurahan(id: e['id'], name: e['name']),
          )
          .toList();
  ...
  Future<List<Kelurahan>> getKelurahanByKecamatanId(int id) async {
    ...
    try {
      Response response = await _dio.get(
          "${dotenv.env['API_HOST']}/sub_district/indistrict?district_id=$id");
      if (response.statusCode != 200) throw Exception();
      var data = response.data as Map<String, dynamic>;
      if (data['status'] == false) throw Exception();
  ...
  Future<List<Kecamatan>> getActiveKecamatan() async {
    ...
    try {
      Response response =
          await _dio.get("${dotenv.env['API_HOST']}/district/active",
              options: Options(
      ...
      var listKecamatan = (data['data'] as List<dynamic>)
